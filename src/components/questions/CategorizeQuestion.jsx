...
} from "../../slices/categorizeQuestionSlice";
...
const CategorizeQuestion = () => {
  ...
  const handleAddQuestion = () => {
    if (
      ...
      data.categories.some(({ answers }) => answers.length > 0)
    ) {
      dispatch(addCategorizeQuestion(data));
      setData({ questionType: "categorize", description: "", categories: [] });
    } else {
  ...
  const handleDragAndDrop = (results) => {
    ...
    if (type === "categories-group") {
      ...
      const [removedDestination] = reorderedCategories.splice(
        categorySourceIndex,
        1
      );
      reorderedCategories.splice(
        categoryDestinationIndex,
        0,
        removedDestination
      );
    ...
    <div className="p-6 max-w-4xl w-3/5 mx-auto rounded-lg mt-10 shadow-xl shadow-stone bg-gray-100">
      <DragDropContext onDragEnd={handleDragAndDrop}>
        <header className="mb-4 text-center">
          <h2 className="font-thin border rounded w-fit px-4 py-1 bg-slate-700 text-white">
            Categorize
          </h2>
        ...
        <section className="mt-4">
                    ...
                    >
                      {(provided) => (
                        <div
                          ...
                          className="flex items-center justify-between mb-2 w-40 border border-slate-400 rounded-md px-2 py-0 "
                          ref={provided.innerRef}
                        >
                          <i className="fa-solid fa-grip-vertical"></i>
                          <p className="text-gray-700 mr-5 flex items-center justify-center">
                            {categoryName}
                          </p>
                          <button
          ...
          <label
            htmlFor="category"
            className="flex items-center justify-flex-end mt-4"
          >
            <input
              type="text"
            ...
            />
            <button
              className="bg-slate-900 text-white rounded-full w-14 p-2.5  ml-4"
              onClick={() => {
                if (categoryInput.categoryName.length) {
                  setData(() => {
                    return {
                      ...data,
                      categories: [...data.categories, categoryInput]
                    };
                  });
                  setCategoryInput({ categoryName: "", answers: [] });
                } else {
      ...
      <section className="my-10">
        ...
        <table className="w-full border border-slate-400 rounded-md">
          <thead>
            <tr>
              <th className="border border-slate-400 rounded-md px-2 py-1">
                Item
              </th>
              <th className="border border-slate-400 rounded-md px-2 py-1">
                Belongs to
                ...
                return (
                  <tr className="mb-10 ">
                          ...
                          setData(() => {
                            return {
                              ...
                              categories: data.categories
                                .reduce((acc, curr) => {
                                  return categoryName === curr.categoryName
                                    ? (acc = [
                                        ...acc,
                                        {
                                          ...curr,
                                          answers: curr.answers.filter(
                                            (ele) => ele !== answer
                                          )
                                        }
                                      ])
                    ...
                    <td className="border border-slate-400 rounded-md">
                      <button
                        className="bg-transparent text-slate-900 rounded-full p-2 hover:opacity-60 flex justify-center items-center w-full "
                        onClick={() => {
                          setData(() => {
                            return {
                              ...data,
                              categories: data.categories.reduce(
                                (acc, curr) => {
                                  return data.categories.indexOf(curr) ===
                                    categoryIndex
                                    ? [
                                        ...acc,
                                        {
        ...
        <label
          htmlFor="answer"
          className="flex items-center justify-flex-start mt-4 ml-2"
        >
          <input
            type="text"
          ...
          <select
            name="answer"
            value={answerInput.categoryName}
            className="px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:border-slate-900 mx-10 cursor-pointer"
            onChange={(event) => {
              setAnswerInput(() => {
          ...
          <button
            className="bg-slate-900 text-white rounded-full w-14 p-2.5 "
            onClick={() => {
              if (
                answerInput.categoryName !== "all" &&
                answerInput.answer.length
              ) {
                setData(() => {
                  return {
                    ...data,
                    categories: data.categories.reduce((acc, curr) => {
                      return answerInput.categoryName === curr.categoryName
                        ? (acc = [
                            ...acc,
                            {
                              ...curr,
                              answers: [...curr.answers, answerInput.answer]
                            }
                          ])
                        : [...acc, curr];
                    }, [])
                ...
                setAnswerInput({ categoryName: "all", answer: "" });
              } else {
      ...
      <button
        className=" text-white rounded px-2.5 py-2 ml-4 bg-blue-500 shadow-lg hover:shadow-indigo-500/40 hover:scale-105 duration-300"
        onClick={() => {
          handleAddQuestion();
        }}
      >
